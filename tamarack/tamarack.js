...
class tkColor {
	...
	contrastWith(_color) {
		return (_color.getLuminance() + 0.05) / (this.getLuminance() + 0.05);
	}

	/**
	 * If the current tkColor:
	 *  - Has a lightness less than or equal to 50% or a saturation less or equal to 0.85
	 *  - A alpha is more or equal to 0.4
	 * @returns {Boolean} True if dark, false otherwise
	 */
	isDark() {
		var white = new tkColor("rgb('255,255,255')");
		
		return this.contrastWith(white) > 1.5;
	}
